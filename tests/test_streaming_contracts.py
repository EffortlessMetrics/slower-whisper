...
- Timestamp format correctness (milliseconds for ts_server, seconds for audio)
...
class TestEventEnvelopeContract:
    ...
    def test_required_fields_always_present(self) -> None:
        ...
        assert "payload" in d
        # schema_version is also always present (Issue #222)
        assert "schema_version" in d

    def test_schema_version_present_and_valid(self) -> None:
        """Test schema_version field is present and follows semver format (Issue #222)."""
        ...
        pattern = r"^\d+\.\d+\.\d+$"
        assert re.match(pattern, d["schema_version"]), (
            f"schema_version must be semver format, got: {d['schema_version']}"
        )

        # Must match the constant
        assert d["schema_version"] == EVENT_ENVELOPE_SCHEMA_VERSION

    def test_schema_version_is_current_version(self) -> None:
        """Test schema_version matches current protocol version (Issue #222)."""

        # Current version should be 2.1.0 or higher
        version_parts = EVENT_ENVELOPE_SCHEMA_VERSION.split(".")
        assert len(version_parts) == 3
        major, minor, _patch = int(version_parts[0]), int(version_parts[1]), int(version_parts[2])

        # Version should be at least 2.1.0 (when schema_version was added)
        assert (major, minor) >= (2, 1), (
            f"Schema version should be >= 2.1.0, got {EVENT_ENVELOPE_SCHEMA_VERSION}"
        )
    ...
    def test_ts_server_is_integer_not_float(self) -> None:
        """Test ts_server is integer (not float) (Issue #222)."""
        envelope = EventEnvelope(
            event_id=1,
            stream_id="str-test",
            type=ServerMessageType.PONG,
        ...
        d = envelope.to_dict()
        assert isinstance(d["ts_server"], int), (
            f"ts_server should be int, got {type(d['ts_server'])}"
        )

    @pytest.mark.asyncio
    async def test_ts_server_from_real_session_is_valid_milliseconds(self) -> None:
        """Test ts_server generated by real session is valid Unix epoch milliseconds (Issue #222)."""
        import time
        ...
        # ts_server should be close to now (within 5 seconds)
        assert abs(d["ts_server"] - now_ms) < 5000, (
            f"ts_server {d['ts_server']} not close to current time {now_ms}"
        )

        # Should be positive
        assert d["ts_server"] > 0

        # Should be an integer
        assert isinstance(d["ts_server"], int)

        # Should be in milliseconds (13+ digits for dates after 2001)
        assert d["ts_server"] > 1000000000000

    def test_ts_audio_fields_are_float_seconds(self) -> None:
        """Test ts_audio_start and ts_audio_end are float seconds (Issue #222)."""
        envelope = EventEnvelope(
            event_id=1,
            stream_id="str-test",
        ...
        # Should be floats (or ints that can represent seconds)
        assert isinstance(d["ts_audio_start"], (int, float))
        assert isinstance(d["ts_audio_end"], (int, float))

        # Values should be reasonable for audio timestamps (in seconds)
        assert d["ts_audio_start"] < 100000, "ts_audio_start should be in seconds, not milliseconds"
        assert d["ts_audio_end"] < 100000, "ts_audio_end should be in seconds, not milliseconds"
    ...
    def test_all_server_message_types_are_documented(self) -> None:
        """Test all ServerMessageType enum values are in the valid_types list (Issue #222)."""
        documented_types = {
            "SESSION_STARTED",
            "PARTIAL",
            "FINALIZED",
        ...
        # Get all enum values
        actual_types = {t.value for t in ServerMessageType}

        # All documented types should exist in the enum
        missing_from_enum = documented_types - actual_types
        assert not missing_from_enum, f"Documented types not in enum: {missing_from_enum}"

        # All enum types should be documented
        undocumented = actual_types - documented_types
        assert not undocumented, f"Enum types not documented: {undocumented}"
...
class TestResumeProtocol:
    ...
    async def test_resume_continues_transcription(self) -> None:
        ...
        # Access the current event ID via the counter's current property
