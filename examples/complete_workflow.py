...
class TranscriptAnalyzer:
    ...
    def __init__(self, json_path: Path, audio_path: Path | None = None):
        """
            ...
            if segment.audio_state:
                ...
                if "pitch" in segment.audio_state:
                    ...
                    if "mean_hz" in pitch_info:
                        ...
                        stats["pitch"]["min"] = min(stats["pitch"]["min"], pitch_info["mean_hz"])
                        stats["pitch"]["max"] = max(stats["pitch"]["max"], pitch_info["mean_hz"])
                ...
                if "energy" in segment.audio_state:
                    ...
                    if "db_rms" in energy_info:
                        ...
                        stats["energy"]["min"] = min(stats["energy"]["min"], energy_info["db_rms"])
                        stats["energy"]["max"] = max(stats["energy"]["max"], energy_info["db_rms"])
    ...
    def find_excited_moments(
        self,
        threshold_energy: str = "high",
        threshold_pitch: str = "high",
        threshold_rate: str = "fast",
    ) -> list[Segment]:
        """
    ...
    def find_calm_moments(
        self,
        threshold_energy: str = "low",
        threshold_pitch: str = "low",
        threshold_rate: str = "slow",
    ) -> list[Segment]:
        """
    ...
    def find_emotional_segments(
        self, min_valence_magnitude: float = 0.3, min_arousal_magnitude: float = 0.3
    ) -> list[Segment]:
        """
        ...
        for segment in self.transcript.segments:
            ...
            if "dimensional" in emotion_data:
                ...
                if (
                    valence_magnitude >= min_valence_magnitude
                    or arousal_magnitude >= min_arousal_magnitude
                ):
    ...
    def find_hesitant_segments(
        self, min_pause_count: int = 2, min_pause_density: float = 0.1
    ) -> list[Segment]:
        """
    ...
    def create_annotated_transcript(self, output_path: Path) -> None:
        ...
        with output_path.open("w", encoding="utf-8") as f:
            ...
            f.write("# Generated from enriched transcript\n\n")
            ...
            for segment in self.transcript.segments:
                ...
                if segment.audio_state:
                    ...
                    if "rate" in segment.audio_state:
                        ...
                        annotations.append(f"Rate: {level} ({syl_per_sec:.2f} syl/sec)")
                    ...
                    if "emotion" in segment.audio_state:
                        ...
                        if "categorical" in emotion:
                            ...
                            annotations.append(f"Emotion: {primary} ({confidence:.1%})")
...
def print_analysis_report(analyzer: TranscriptAnalyzer) -> None:
    ...
    if audio_stats["energy"]["mean"] != 0:
        print("\nEnergy Analysis:")
        print(
            f"  Range: {audio_stats['energy']['min']:.1f} - {audio_stats['energy']['max']:.1f} dB"
        )
    ...
    if audio_stats["rate"]["mean"] != 0:
        print("\nSpeech Rate Analysis:")
        print(
            f"  Range: {audio_stats['rate']['min']:.2f} - {audio_stats['rate']['max']:.2f} syl/sec"
        )
...
def main():
    ...
    parser.add_argument(
        "--output-annotated", type=Path, help="Output path for annotated transcript"
    )
    parser.add_argument("--output-csv", type=Path, help="Output path for CSV export")
