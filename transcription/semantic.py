    ...
    def annotate(self, transcript: Transcript) -> Transcript:
        ...
        for segment in transcript.segments:
            ...
            for keyword, kw_lower, pattern in self._escalation_patterns:
                # Fast-path literal check: only run expensive regex if the exact lowercase string
                # is present in the text. This is safe because our regex strictly requires the
                # literal string to match (fuzzy matching/variations are not used).
