...
def format_doctor_report(report: DoctorReport, use_color: bool = True) -> str:
    ...
    # Status symbols
    symbols = {
        CheckStatus.PASS: (Colors.green(Symbols.CHECK) if use_color else Symbols.PASS_ASCII),
        CheckStatus.WARN: (Colors.yellow(Symbols.WARN) if use_color else Symbols.WARN_ASCII),
        CheckStatus.FAIL: (Colors.red(Symbols.CROSS) if use_color else Symbols.CROSS_ASCII),
        CheckStatus.SKIP: (Colors.dim(Symbols.SKIP) if use_color else Symbols.SKIP_ASCII),
    }
