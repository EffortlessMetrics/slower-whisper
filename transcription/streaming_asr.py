...
class StreamingASRAdapter:
    ...
    def _detect_speech_frames(self, audio: np.ndarray, frame_size_ms: int = 30) -> list[bool]:
        ...
        # Truncate audio to match full frames to reshape
        audio_frames = audio[: num_frames * frame_size]

        # Reshape into frames and calculate RMS energy vectorized across axis 1
        reshaped = audio_frames.reshape(num_frames, frame_size)

        # Calculate RMS energy for each frame, ensuring cast to float32 to prevent overflow
        energies = np.sqrt(np.mean(reshaped.astype(np.float32) ** 2, axis=1))
