...
def _normalize_one_file(
    ...
    """
    # Note: timestamp checks are now done in normalize_all() before dispatching tasks.
    # However, if dst exists here, it means we decided to overwrite it.
...
def normalize_all(paths: Paths) -> None:
    ...
    for src in sorted(paths.raw_dir.iterdir()):
        if src.is_file():
            ...
            # Check timestamps before adding to process list to avoid thread overhead
            if dst.exists():
                try:
    ...
    if not files_to_process:
        if skipped_count == 0:
            ...
            logger.info("All files are already normalized")
        return
