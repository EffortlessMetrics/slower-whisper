...
def normalize_all(paths: Paths) -> None:
    ...
    for src in sorted(paths.raw_dir.iterdir()):
        if src.is_file():
            ...
            # Check if already up-to-date before submitting to executor
            # This saves significant overhead for large batches of existing files
            if dst.exists():
                try:
                    src_mtime = src.stat().st_mtime
                    dst_mtime = dst.stat().st_mtime
                    ...
                    # If stat fails, fall through to safe processing path
                    pass
    ...
    if not files_to_process:
        if skipped_count == 0:
            ...
            logger.info("All audio files are already normalized")
        return
