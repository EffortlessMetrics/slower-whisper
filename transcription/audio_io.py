...
def normalize_all(paths: Paths) -> None:
    ...
    for src in sorted(paths.raw_dir.iterdir()):
        ...
        # Check optimization: skip files that are already up-to-date
        # This avoids submitting tasks to the thread pool for files that don't need processing
        if dst.exists():
            try:
                src_mtime = src.stat().st_mtime
                dst_mtime = dst.stat().st_mtime
                ...
                # If stat fails, fall back to processing the file safely in the worker
                pass
